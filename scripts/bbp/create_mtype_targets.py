...
def create_targets(blue_config, output,
    ...
    for ntag, mtype in enumerate(mtypes):
        ...
        sampled_mtypes = random.sample(set(mtype_cells),
            int((len(mtype_cells) * percent / 100.0)))
        exemplars_list.append(sampled_mtypes)
    ...
    for i, mtype in enumerate(mtypes):
        voxelization_target_output = \
            open("%s/%s_%s_%s.list" % (output, target, mtype, str(percent)),
                'w')
        default_target_output = open("%s/%s_%s.target" %
                                     (output, mtype, str(percent)), 'w')
        default_target_output.write(
            "Target Cell %s_%s_%sp \n" % (target, mtype, str(percent)))
        default_target_output.write("{\n")
        gids_string = ''
    ...
    voxelization_target_output = open("%s/%s_mtypes_%s.list" %
                                      (output, target, str(percent)), 'w')
    default_target_output = open(
        "%s/mtypes_%s_%s.target" % (output, target, str(percent)), 'w')
    default_target_output.write(
        "Target Cell mtypes_%s \n" % str(percent))
    default_target_output.write("{\n")
    gids_string = ''
    for item in exemplars_list:
        for data in item:
