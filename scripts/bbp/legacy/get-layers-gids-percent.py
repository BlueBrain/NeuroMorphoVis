...
def select_neurons(blue_config, target, percent=100.0):
    ...
    for i in range(1, 7):
        list_output = open("Layer%d_%s-%dp.list" % (i, target, int(percent)), 'w')
        target_output = open("Layer%d_%s-%dp.target" % (i, target, int(percent)), 'w')
        ...
        target_output.write("Target Cell Layer%d_%s-%dp \n" % (i, target, int(percent)))
        target_output.write("{\n")
        gids_string = ''
