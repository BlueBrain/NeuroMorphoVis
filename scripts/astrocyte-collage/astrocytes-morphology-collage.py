...
# @load_morphologies
####################################################################################################
def load_meshes(meshes_directory):
    ...
    for mesh_file in mesh_files:
        ...
        # Load the mesh
        scene_objects = nmv.file.import_object_from_blend_file(meshes_directory, mesh_file)
        
        # Get the mesh from all the objects in the scene
        for scene_object in scene_objects:
            if scene_object.type == 'CURVE' or scene_object.type == 'MESH':

                # Adjust it 
                nmv.scene.select_object(scene_object)
                nmv.scene.set_active_object(scene_object)
                ...
                nmv.scene.deselect_object(scene_object)
...
if __name__ == "__main__":
    ...
    mtypes = os.listdir(args.meshes_directory)

    # Sort the mtypes
    mtypes.sort()

    # Load the meshes from the blend files
    meshes = load_meshes(args.meshes_directory)
