...
class MetaRandomSpineBuilder:
    ...
    def __init__(self,
        ...
        # Morphology, and make sure its a deep copy to avoid affecting the original morphology
        self.morphology = morphology

        # Loaded options from NeuroMorphoVis
        self.options = options

        # A list containing all the spines morphologies
        self.spine_morphologies = None
    ...
    def build_spine(self):
        ...
        spine_mesh = nmv.scene.ops.convert_object_to_mesh(spine_polyline)
        ...
        if not self.options.morphology.ignore_apical_dendrites:
            if self.morphology.has_apical_dendrites():
                for i, arbor in enumerate(self.morphology.apical_dendrites):
                    ...
                    spines_locations_and_normals = nmv.skeleton.ops.apply_operation_to_arbor(
                        *[arbor,
                          nmv.skeleton.ops.get_random_spines_locations_and_normals_across_segments,
                          number_of_spines_per_segment])

                    for spine in spines_locations_and_normals:

                        p0 = spine[0]
                        normal = spine[1]
                        p2 = p0 + normal * 1.0

                        nmv.geometry.draw_line(point1=p0, point2=p2)
