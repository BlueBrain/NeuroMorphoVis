...
def kernel_minimum_section_volume(morphology):
    ...
    morphology.stats.minimum_section_volume = copy.deepcopy(min(combined_results))
    ...
    segment_results = nmv.analysis.invoke_kernel(
        morphology,
        nmv.analysis.compute_minimum_segment_volume,
        nmv.analysis.compute_minimum_analysis_result_of_morphology)
    ...
    # Store the result in the morphology
    morphology.stats.minimum_segment_volume = copy.deepcopy(min(combined_results))
...
def kernel_maximum_section_volume(morphology):
    ...
    # Store the result in the morphology
    morphology.stats.maximum_section_volume = max(combined_results)

    # NOTE: We will also compute the minimum segment surface area to complete the analysis
    segment_results = nmv.analysis.invoke_kernel(
        morphology,
        nmv.analysis.compute_maximum_segment_volume,
        nmv.analysis.compute_maximum_analysis_result_of_morphology)
    ...
    # Store the result in the morphology
    morphology.stats.maximum_segment_volume = copy.deepcopy(max(combined_results))
