...
class BoundingBox():
    ...
    def __init__(self, 
        ...
        Constructor
        """
        if p_min != Vector((0, 0, 0)) and max != Vector((0, 0, 0)):
            self.p_min = p_min
            self.p_max = p_max
    ...
    # @print_details
    ################################################################################################
    def print_details(self, name='bounding box'):
        """
...
def extend_bounding_boxes(bounding_boxes_list):
    """
    Returns the largest bounding box that is composed of smaller ones.
    ...
    p_min = Vector((1e32, 1e32, 1e32))
    p_max = Vector((-1e32, -1e32, -1e32))
    ...
    bounding_box = BoundingBox(p_min, p_max)
...
def get_object_bounding_box(scene_object):
    """
    Returns the bounding box of a given object in the scene.
    ...
    # Build bounding box object
    bounding_box = BoundingBox(center=center, bounds=bounds)
...
def confirm_object_bounding_box(scene_object):
    ...
    # Build bounding box object
    bbox = BoundingBox(p_min=p_min, p_max=p_max)
...
def compute_scene_bounding_box():
    """
    Gets the bounding box of all the objects in the scene.
...
def compute_unified_bounding_box(non_unified_bounding_box):
    """
    Computes a unified bounding box from a non unified one, where all the dimensions are set to
    ...
    unified_bounding_box = BoundingBox(center=non_unified_bounding_box.center,
...
def compute_360_bounding_box(non_unified_bounding_box,
    ...
    """
    Computes a specific bounding box from a non unified one, where all the XZ dimensions are set to
    ...
    y_max = soma_center[1] + (non_unified_bounding_box.bounds[1] / 2.0)
    z_max = soma_center[2] + diagonal
    x_min = soma_center[0] - diagonal
    y_min = soma_center[1] - (non_unified_bounding_box.bounds[1] / 2.0)
...
def draw_scene_bounding_box():
    """
    Computes the scene bounding box and draws it.
...
def draw_bounding_box(bbox,
    ...
    bbox_mesh = mesh_ops.join_mesh_objects(line_list, name=name)
