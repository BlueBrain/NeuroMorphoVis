...
import scene_ops
import sphere_ops
import morphology_polyline_ops
import mesh_ops
...
def is_arbor_disconnected_from_soma(arbor,
                                    threshold=15.0):
    """
    This function checks if a branch is disconnected from the soma or not based on a given
    threshold distance.
...
def find_nearest_apical_dendritic_sample_to_axon(morphology,
                                                 nearest_sample=None):
    """
    This function finds the nearest sample along the apical dendrite to the axon.
...
def find_nearest_basal_dendritic_sample_to_axon(morphology,
                                                nearest_sample=None):
    """
    This function finds the nearest sample along the basal dendrites to the axon.
...
def find_nearest_dendritic_sample_to_axon(morphology):
    """
    This function finds the nearest dendrite to the axon in case if the axon was disconnected
...
def find_nearest_apical_dendritic_sample_to_basal_dendrite(morphology,
                                                           basal_dendrite):
    """
    Finds the nearest sample on the apical dendrite to the given basal dendrite.
...
def find_nearest_basal_dendritic_sample_to_basal_dendrite(morphology,
                                                          basal_dendrite):
    """
    Finds the nearest sample on a basal dendrite to the given basal dendrite.
...
def verify_axon_connection_to_soma(morphology):
    """
    Verifies if the axon of a morphology is connected to its soma or not.
    ...
    if is_arbor_disconnected_from_soma(morphology.axon):
        ...
        # Report the repair
        nmv.logger.log('\t\t* REPAIRING: The axon is re-connected to section [%d, %s] @ sample [%s]'
    ...
    nmv.logger.log('\t\t* NOTE: The axon @ section [%d] is connected to the soma' % morphology.axon.id)
...
def update_arbors_connection_to_soma(morphology):
    """
    Verifies if the different arbors of the morphology are connected to the soma or not.
    The apical dendrite is always assumed to be connected to the soma since its large.
    ...
    # Verift the connectivity of the axon
...
def connect_arbor_to_soma(soma_mesh,
    ...
    mesh_ops.rotate_face_towards_point(clipping_plane, Vector((0, 0, 0)))

    # Clip the arbor mesh and return a reference to the result
    section_mesh = mesh_ops.intersect_mesh_objects(arbor.mesh, clipping_plane)
    ...
    soma_mesh_face_index = mesh_ops.get_index_of_nearest_face_to_point(
    ...
    mesh_ops.select_face_vertices(soma_mesh, soma_mesh_face_index)

    # Select the section mesh
    nmv.scene.ops.select_objects([arbor.mesh])

    # Deselect all the vertices of the section mesh, for safety !
    mesh_ops.deselect_all_vertices(arbor.mesh)

    # Get the nearest face on the section mesh
    section_face_index = mesh_ops.get_index_of_nearest_face_to_point(
        arbor.mesh, intersection_point)

    # Select the face
    mesh_ops.select_face_vertices(arbor.mesh, section_face_index)

    # Apply a joint operation, for the moment, use 'neuron' as the mesh object, since it will
    # be changed later
    soma_mesh = mesh_ops.join_mesh_objects([soma_mesh, arbor.mesh], name=soma_mesh.name)
    ...
    mesh_ops.select_all_vertices(soma_mesh)

    # Deselect all the vertices of the parent mesh, for safety reasons
    mesh_ops.deselect_all_vertices(soma_mesh)
...
def is_point_located_within_extents(point,
    ...
    for extent in extents:
        ...
        if sphere_ops.is_point_inside_sphere(extent_center, extent_radius, point):
            return True
...
def update_secondary_arbor_starting_point_to_avoid_intersection(secondary_poly_line,
    ...
    secondary_poly_line_length = morphology_polyline_ops.get_poly_line_length(secondary_poly_line)

    first_distance = secondary_poly_line.data.splines.active.points[0].radius * 2


    # check in advance if the distance works or not
    initial_starting_factor = morphology_polyline_ops.get_poly_line_normalized_start_factor(
    ...
    if morphology_polyline_ops.poly_line_intersect_mesh(
        ...
        for i in range(1, 100):
            ...
            starting_factor = morphology_polyline_ops.get_poly_line_normalized_start_factor(
...
####################################################################################################
# @bridge_secondary_arbor_to_primary_mesh
####################################################################################################
def bridge_secondary_arbor_to_primary_mesh(secondary_arbor,
                                           primary_mesh,
                                           delta):
...
def bridge_arbor_poly_line_to_skeleton_mesh(arbor_poly_line,
    ...
    :param secondary_arbor_poly_line:
    :param skeleton_mesh:
    ...
    indices = mesh_ops.get_indices_of_nearest_faces_to_point_within_delta(
        mesh_object_1, connecting_point[0])
    nearest_face_index_on_mesh_1 = mesh_ops.get_index_of_nearest_face_to_point_in_faces(
    ...
    # Select this face
    mesh_ops.select_face_vertices(mesh_object_1, nearest_face_index_on_mesh_1)
    ...
    # Close all the open faces (including the caps) to ensure that there are no holes in the mesh
    mesh_ops.close_open_faces(mesh_object_2)

    # Get the nearest face to the bridging point
    nearest_face_index_on_mesh_2 = mesh_ops.get_index_of_nearest_face_to_point(
        mesh_object_2, connecting_point[1])

    # Select this face
    mesh_ops.select_face_vertices(mesh_object_2, nearest_face_index_on_mesh_2)
    ...
    mesh_ops.deselect_all_vertices(mesh_object_1)
