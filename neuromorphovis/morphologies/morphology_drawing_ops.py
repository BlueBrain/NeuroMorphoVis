...
def draw_section_from_poly_line_data(data,
                                     ...
                                     caps=False):
    """
    Draws a morphological section as a poly-line (or tube) object and returns a reference to it.
    ...
    section_object = line_ops.draw_poly_line(poly_line_data=data,
                                             name=section_name,
                                             material=material,
                                             color=color,
...
def draw_connected_sections(section,
    ...
    """
    Draws a list of sections connected together as a poly-line.
    ...
    section_data = morphology_polyline_ops.get_connected_sections_poly_line(
        section=section,
        is_continuous=is_continuous,
        is_last_section=is_last_section,
        fixed_radius=fixed_radius,
        transform=transform,
        process_terminals=repair_morphology,
...
def draw_disconnected_skeleton_sections(section,
    ...
    """
    Draws a list of sections connected together as a poly-line, however, disconnected at the
    secondary branches.
    ...
    # Get a list of all the poly-line that corresponds to the given section
    section_data = morphology_polyline_ops.get_disconnected_skeleton_sections_poly_line(
        section=section,
        is_continuous=is_continuous,
        is_last_section=is_last_section,
        fixed_radius=fixed_radius,
...
def draw_disconnected_segments(section,
                               ...
                               camera=None):
    """
    Draws a given morphological section as a series of disconnected segments, where each segment is
    represented by a unique cylinder.
    ...
    section_data = morphology_polyline_ops.get_section_poly_line(
    ...
    for i in range(len(section_data) - 1):
        ...
        # Draw the segment
        created_segments = line_ops.draw_poly_line(
...
def visualize_extrusion_connection(point,
    ...
    the same radius of the branch at the extrusion point along the connection between the soma
    and the branch.
    ...
    connection_sphere = mesh_objects.create_uv_sphere(
