...
# @InputOutputData
####################################################################################################
class InputOutputData(bpy.types.Panel):
    ...
    bpy.types.Scene.InputSource = EnumProperty(
        items=[(enumerators.__input_h5_swc_file__, 'H5 or SWC File',
                'Load individual h5 or swc file without a circuit'),
               (enumerators.__input_circuit_gid__, 'BBP Circuit (GID)',
                'Load a specific GID from the circuit config')], 
        name="Input Source",
        default=enumerators.__input_h5_swc_file__)
    ...
    bpy.types.Scene.CircuitFile = StringProperty(
        ...
        default="Select Circuit File", maxlen=2048, subtype='DIR_PATH')
    ...
    def draw(self, context):
        ...
        if 'Select Directory' in scene.OutputDirectory:
            ui_interface.options.output.output_directory = None
        else:
            ui_interface.options.output.output_directory = \
                scene.OutputDirectory
            ui_interface.options.output.images_directory = \
            ...
            ui_interface.options.output.morphologies_directory = \
                '%s/%s' % (scene.OutputDirectory, scene.MorphologiesPath)
            ui_interface.options.output.meshes_directory = \
                '%s/%s' % (scene.OutputDirectory, scene.MeshesPath)
            ui_interface.options.output.analysis_directory = \
...
def register_panel():
    ...
    # InputOutput data
    bpy.utils.register_class(InputOutputData)
...
def unregister_panel():
    ...
    # InputOutput data
    bpy.utils.unregister_class(InputOutputData)
